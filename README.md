...
>1. 采控类型：根据该井的实际采集数据，选择采控配置模块中创建的类型；  
...
&emsp;&emsp;点击“创建”按钮创建采控类型，如类型名称“类型一”，类型编码“type1”，类型描述“抽油机全部数据”，创建完成后，在右侧列表中勾选实际采集项和控制项，点击“保存”完成配置。后续在**井名信息**模块数据录入时按照单井实际采控项选择不同的采控类型。
