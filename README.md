...
系统适用于Windows、Linux、Mac64位及32位操作系统，请在购买时注明所需部署机器的版本型号、IP地址以及网卡物理地址。
...
公有云：**http://39.98.134.121:16100/ap**  
用户名、密码请咨询本公司
...
&emsp;&emsp;通过点击界面中缝位置的图标![页面布局](https://github.com/cosog-chentr/apmd/blob/master/Image/PNG/025.png?raw=true)或![页面布局](https://github.com/cosog-chentr/apmd/blob/master/Image/PNG/026.png?raw=true)可实现界面伸缩。
...
1. 统计子标签：各主标签包含的子项，如平衡包括电流平衡和功率平衡；  
1. 统计表/图：根据选择的各主标签、子标签显示相关统计信息。
...
1. 统计表中显示该组织下的所有“游动凡尔失灵”井；  
1. 右侧单井详情中显示该井的详细信息；  
1. 选择一行后点击**查看历史**或**双击**该行可查看该井的历史信息。
...
（5）最小载荷  
&emsp;&emsp;功图最小载荷  
![](https://github.com/cosog-chentr/apmd/blob/master/Image/PNG/027.png?raw=true) 
...
>1. 井名；  
>1. 举升类型，包括抽油机、螺杆泵；  
>1. 驱动名称，包括A11协议、四化协议等，根据实际协议选择；  
>1. 采集类型，根据该井的实际采集数据选择采集数据模块中创建的类型；  
>1. 设备地址，网关的MAC地址或SIM卡号码；  
>1. 设备编号，网关只连接一口井时，填写01；对于丛式井，网关连接多口井时，按网关中配置的设备ID填写，如01、02、03......；  
>1. 功图采集间隔，现场功图采集的时间间隔；  
>1. 状态，时间是否已同步至前端设备，包括等待下发、已同步；  
>1. 离散采集间隔，离散数据的采集间隔，如电流、电压等；  
>1. 数据保存间隔，数据保存进数据库的时间间隔；  
>1. 时率来源，DI信号（连接DI线）、电参计算（采集电参数据）、转速计算（螺杆泵井有转速数据可选择转速计算）、人工录入；  
>1. 视频路径，监控视频的URL路径；  
>1. 排序编号：井名在系统显示时的排序。
...
**注意：**  
**（1）录入时需要先在组织导航中选择录入井所在的组织，如四矿；**  
**（2）复制时序号行不要复制。**

![](https://github.com/cosog-chentr/apmd/blob/master/Image/GIF/005.gif?raw=true)
...
![](https://github.com/cosog-chentr/apmd/blob/master/Image/GIF/006.gif?raw=true)

#### 3.8.1.4 删除数据

选中该行或多行，右键，选择删除行，然后点击“保存”按钮。

![](https://github.com/cosog-chentr/apmd/blob/master/Image/GIF/007.gif?raw=true)

### 3.8.2 生产数据

#### 3.8.2.1 数据收集

1、生产时间，时率来源为人工录入时填写，全天工作填写24，间抽按间抽时间填写，停井填写0；  
2、原油密度、水密度、天然气相对密度、饱和压力、油层中部深度、油层中部温度如果没有单井数据，可以参考所在区块数据；  
3、油压，未测油压数据可填写回压；  
4、泵级别，1、2、3；  
5、泵径，国标：28，32，38，44，51，57，63，70，83，95；API标准：26.99、31.8、38.1、44.5、45.2、50.8、57.2、63.5、69.9、95.3；  
6、套管内径，生产套管内径；  
7、抽油杆数据，  
-  （1）抽油杆数据不包括光杆、泵上拉杆；  
...
收集完成后，在Excel整理完成，复制粘贴到模块中。粘贴时无需按列表中顺序调整Excel表，直接粘贴保存即可。

![](https://github.com/cosog-chentr/apmd/blob/master/Image/GIF/008.gif?raw=true)

### 3.9.3 采控配置

针对不同的设备安装情况设置不同的采集项和控制项，  
**采集项包括：**  
（1）运行状态；  
（2）生产数据，包括油压、套压、回压、井口油温、动液面、含水率；  
（3）电参数据，A/B/C相电流、A/B/C相电压、有功功耗、无功功耗、有功功率、无功功率、反向功率、功率因数；  
（4）变频数据，变频运行频率；  
（5）功图数据，采集时间、功图点数、冲次、冲程、曲线位移值、曲线载荷值、曲线电流值、曲线功率值、功图采集间隔、功图采集设置点数；  
（6）螺杆泵特有数据，转速、转矩；  
（7）平衡调节，远程调节状态。  
**控制项包括：**  
（1）启停抽；  
（2）变频设备频率；  
（3）数字抽油机参数，远程调节触发控制、平衡计算方式、重心远离支点调节时间、重心接近支点调节时间、参与平衡计算冲程次数、平衡调节上限、平衡调节下限、重心远离支点每拍调节时间、重心接近支点每拍调节时间。

点击“创建”按钮创建采控类型，如类型名称“类型一”，类型编码“type1”，类型描述“抽油机全部数据”，创建完成后，在右侧列表中勾选实际采集项和控制项，点击“保存”完成配置。后续在**井名信息**模块数据录入时按照单井实际采控项选择不同的采控类型。

![](https://github.com/cosog-chentr/apmd/blob/master/Image/PNG/017.png?raw=true)
...
在“生产数据”模块中修改参数只会应用到之后的计算分析中，对已经计算完成的历史数据并不生效。如果想要对历史计算结果修改生产数据重新计算，需要在“计算维护”模块中进行。  
“计算维护”模块中显示的是选中组织下的历史功图计算结果及计算使用的生产数据。  
其中计算状态0：未计算；1：计算成功；-99：数据校验失败（生产数据存在错误）。  
注意：历史结果会被重新计算的结果覆盖。  
对历史结果进行重新计算有两种方法，根据实际情况选择较为便捷的方法：

1、修改数据计算  
选择目标井，找到需要修改的某条记录或某个时间段的多条记录，修改完生产数据后，点击“修改数据计算”按钮进行重新计算。  
例如：修改S68112井，2019年2月24号全部功图采集时间的泵径数据并重新计算。  
操作：井名中选择S68112，时间范围选择2019-02-24至2019-02-24，列表中将泵径38mm修改为44mm，点击“修改数据计算”按钮。

![](https://github.com/cosog-chentr/apmd/blob/master/Image/PNG/018.png?raw=true)

2、关联数据计算  
利用生产数据表中的最新生产数据对某口井或所有井某个时间段内的所有历史结果进行重新计算。  
（1）单井关联数据计算  
例如：修改S68112井，2019年1月24日至2019年2月24日内的所有历史结果。  
操作：“生产数据”模块中已录入最新生产数据，“计算维护”模块中井名选择S68112，时间范围2019-01-24至2019-02-24，点击“关联数据计算”。弹出操作确认对话框，确认无误后点击“是”。这时，S68112井2019年1月24日至2019年2月24日的所有历史数据将按最新的生产数据进行重新计算。

![](https://github.com/cosog-chentr/apmd/blob/master/Image/PNG/019.png?raw=true)

（2）所有井关联数据计算  
例如：单位组织下所有井，2019年1月24日至2019年2月24日内的所有历史结果。  
操作：“生产数据”模块中已录入最新生产数据，“计算维护”模块中井名不选择或选择全部，时间范围2019-01-24至2019-02-24，点击“关联数据计算”。弹出操作确认对话框，确认无误后点击“是”。这时，所有井2019年1月24日至2019年2月24日的所有历史数据将按最新的生产数据进行重新计算。
...
对模块进行编辑，生成功能导航，控制功能模块显示、隐藏等。

![](https://github.com/cosog-chentr/apmd/blob/master/Image/PNG/021.png?raw=true)

### 3.9.2 字典配置

对各模块显示的字段进行管理，主要修改字段名称、显示顺序、是否显示等。

![](https://github.com/cosog-chentr/apmd/blob/master/Image/PNG/022.png?raw=true)

### 3.9.3 统计配置

配置各项参数的统计级别。

![](https://github.com/cosog-chentr/apmd/blob/master/Image/PNG/023.png?raw=true)

### 3.9.4 报警配置

设置工况类型的报警类型、报警项、级别、开关、颜色等。

![](https://github.com/cosog-chentr/apmd/blob/master/Image/PNG/024.png?raw=true)